...
Page(extend({}, Dialog, {
  ...
  orderlist: function () {
    ...
    app.fetch.newData.result(url).then(res => {
      var infos = res.data.response.responseBody;
      for (var i in infos) {
        if (infos[i].date && infos[i].endTime) {
          infos[i].dateStr = infos[i].date.substring(5) + '  ' + infos[i].endTime;//预约过期时间处理
          infos[i].dataInfo = infos[i].date.substring(5);
        }
      }
      if (infos){
        this.setData({
