    ...
    app.fetch.newData.result({
      ...
      if ((res.data.response.responseHeader.code == 200) && res.data.response.responseBody && res.data.response.responseBody.list.length) {
        ...
        var data = that.data.jmlist,
          i = that.data.options.index,
          kankid
        if (that.data.options.kankeId) {
          kankid = that.data.options.kankeId.length > 0 ? that.data.options.kankeId : null
          console.info(kankid, typeof (kankid))
          if (typeof (kankid) == 'string') {
            data.forEach(function (key, value) {
              if (key.kankeId == kankid) i = value
              return false
            })
          }
        }
      ...
      switch (that.data.options.flag) {
        case '0':
          wx.setNavigationBarTitle({
            title: that.data.title + '-回看'
          });
          break;
        case '1':
          wx.setNavigationBarTitle({
            title: '正在直播：' + that.data.title
          });
          break;
        case '2':
          wx.setNavigationBarTitle({
            title: that.data.title + '-即将播放'
          });
          break;
        default:
      }
